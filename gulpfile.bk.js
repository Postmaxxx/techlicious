...
/*
gulp.task('build', (cb) => {
    runSequence(['clean', 'sass', 'templates', 'scripts', 'vendors', 'purgecss','images'], cb);
});
*/
...
gulp.task('style-vendors', () => {
    const plugins = [tailwindcss(src.configFolder + 'tailwind.config.js')];
    const tailwind = gulp.src('./node_modules/tailwindcss/tailwind.css')
...
*/
    gulp.src(dist.cssDir + '/tailwind.min.css')
        .pipe(
            purgecss({
                content: [
                    src.templatesAndIncludes,
                    src.scriptsFolder + '/common.js'
                ],
...
gulp.task('watchme', () => {
    ...
    watch(src.templatesAndIncludes, ['templates']);
    watch(src.scripts, ['scripts']);
    watch(src.configFolder + 'tailwindConfig.js', ['style-vendors']);
});
...
gulp.task('vendors', gulp.series ('copy-assets', 'style-vendors', 'script-vendors',
    function (done) { done(); }
));


gulp.task('build', gulp.series (gulp.parallel('clean', 'sass', 'templates', 'scripts', 'vendors', 'purgecss','images'),
    function (done) { done(); }
));

gulp.task('default', gulp.series( gulp.parallel('build', 'serve', 'images'), 'watchme',
    function (done) { done(); }
));
...
/*
gulp.task('vendors', () => {
    gulp.series('copy-assets', 'style-vendors', 'script-vendors');
});
*/
